...
- ~~Recuperar `BuildingPreview.ts` para la Fase 5~~ → **resuelto**: el huérfano estaba rancio (firma con `animatingWalls`, eliminado en Fase 2), así que la Fase 5 extrajo el inline vivo en vez de adoptarlo. `BuildingPreview.ts` queda obsoleto en el historial.
