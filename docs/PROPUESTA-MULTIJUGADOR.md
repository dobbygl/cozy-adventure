...
- **Despliegue: standalone.** El juego sale del iframe del playground. Control total de red y CSP, transporte realtime sin intermediarios. Esto elimina el mayor bloqueante potencial (que la CSP del host bloquease conexiones salientes), pero implica resolver fuera lo que el host daba gratis: servir el bundle, autenticar y, sobre todo, la grabación de canvas y el puente con el LLM que vivían en `ScriptsLoader-Universal.js` vía `postMessage`. ✅ **Hecho (2026-06-28):** la salida del iframe ya se ejecutó (commit `92037ee`); los scripts host se **retiraron** (no se reimplementaron). Si la grabación de canvas o el puente con el LLM vuelven a hacer falta, se añaden como módulo propio bajo `src/` o como endpoint, no como puente host.
...
- ~~**Funciones del host huérfanas al salir del iframe:** grabación de canvas y puente con el LLM. Hay que decidir si se reimplementan como endpoints propios o se retiran.~~ ✅ **Resuelto (2026-06-28):** se **retiraron** (scripts host borrados). Se reañadirán como módulo/endpoint propio solo si vuelven a hacer falta.
