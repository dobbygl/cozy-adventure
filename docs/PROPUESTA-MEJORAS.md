...
> **Actualización 2026-06-28.** Este documento describe el estado previo a la salida del iframe del playground. Quedan **superados** de forma transversal todos los puntos que asumen contexto iframe/host: el juego ya corre **standalone como PWA** (puente host de Rosebud borrado, commit `92037ee`) y el guardado pasó de **cookies+chunking a una sola escritura en `localStorage`** (`4a16f57`/`11befc6`/`b571f4d`). En concreto dejan de aplicar: el item P0 de "verificar storage dentro del iframe", el de "scripts del host latentes", el `@import` de fuente bajo CSP del host y las notas sobre rutas absolutas de los 5 scripts host. **El resto sigue vigente.** La auditoría a nivel de código con el detalle de lo pendiente está en `docs/INFORME-TECNICO-2026-06.md` (con su propia actualización del 2026-06-28).
