...
- **Companions:** `src/companion/` is a small framework so the dog can scale to other companions. `Companion` (abstract base) owns the shared shell: model load (+ fallback box) driven by a `CompanionDefinition` (model/scale/speeds/distances; `DOG_DEFINITION` is the dog's), the reusable collaborators `CompanionAnimator` (mixer + clip aliases idle/walk/bark), `CompanionPhysics` (gravity + ground following), `CompanionLocomotion` (move + turn), the per-frame preamble, and teardown; subclasses fill the hooks `updateBehavior()`/`onLoaded()`/`onDestroy()`. `DogCompanion extends Companion` and holds only the dog brain: the follow/wander/fetch/idle state machine, the carry list (`fetch/CarryVisual`), `fetch/DeliveryService` (inventory grant + popups) and `fetch/fetchQueries` (pure nearest/drop-age helpers). The companion reaches the rest of the game **only through ports** (`companion/ports.ts`: `GroundProvider`, `InventoryPort`, `CompanionUiPort`, `DroppedItemsPort`), implemented in `companion/gameInstancePorts.ts` and resolved through an injected `getGame` getter (`Game` passes `() => this`; defaults to `window.gameInstance`) so the live, possibly-swapped systems are seen each frame. The collaborators + ports are unit-tested headlessly (`test/companion/`). **Single-player only today; runs in network mode too — see the gotcha below.**
...
- **Inter-system ordering sometimes relies on `setTimeout`** as a sync hack (e.g. in `game.ts`, `DogCompanion.bark()` + `companion/gameInstancePorts.ts`, `BuildingSystem`). These are timing-fragile; if you touch initialization order, prefer awaiting the real load/init events.
- **The dog is fully server-authoritative in network mode (decisions + position + pickups).** Single-player runs the local `updateBehavior` brain unchanged. In network mode the **server** owns the dog: `apps/server/src/world/dog.ts` `DogSimulation` keeps one dog per player, steps its position toward a target every tick (server.ts `runDog`, every tick now, not 1 Hz), picks up the nearest leashed drop on arrival (emits `drop_removed`/`byDogOf` + forwards the owner an `inventory_delta`), then returns to the owner to "deliver". The server pushes the owner a `dog_state` message (protocol **v6**) = `{ position, target, carrying }`, sent on change + a low-rate heartbeat (a **waypoint**, not a 15 Hz path like avatars). The client renders it via DI: `Companion.setMovement()` swaps the local brain for `ServerDrivenMovement` (created in `Game.initializeDogCompanion` when `sessionMode === 'network'`, fed by the `onDogState` handler), which steers the mesh toward `target` (or the owner when null), eases toward the authoritative `position` (interpolation), plays walk/idle, and mirrors the mouth carry count. So the client dog **never mutates the world**; `InventoryPort.addItem`/`DroppedItemsPort.remove` are single-player-only (the MP brain that called them is bypassed). Each client's dog is owner-private (peers don't see it yet — replication is a follow-up). Grant lands at grab; the carry/deliver is a server-driven visual.
