...
- **The dog companion runs in network mode too, and mutates local state there.** `Game.initializeDogCompanion()` is called unconditionally in `startGame()` (no `sessionMode` guard), and the dog's `DeliveryService` (`InventoryPort.addItem`) and drop removal (`DroppedItemsPort.remove`) mutate the **local** inventory and drop arrays. In multiplayer that is a latent **desync** (it bypasses the command→confirmed-event path every other world mutation uses). The companion ports localize the fix to one spot (route `InventoryPort.addItem`/`DroppedItemsPort.remove` through commands, or guard the dog to single-player), but it is **not done yet** — left out of the companion refactor to keep that change behavior-preserving.
