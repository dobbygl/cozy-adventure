...
export class BuildingSaveManager {
  ...
  getAllBuiltObjectsForSaving() {
    ...
    Object.entries(builtObjectsByType).forEach(([registryType, objects]) => {
      ...
      objects.forEach((object, index) => {
        ...
        if (object.userData && object.userData.buildingType && 
            object.userData.buildingType !== registryType) {
          console.log(`Object ${index}: userData.buildingType (${object.userData.buildingType}) differs from registry type (${registryType})`);
          buildingType = object.userData.buildingType;
        } else if (object.userData && object.userData.type && 
                   object.userData.type !== registryType) {
          console.log(`Object ${index}: userData.type (${object.userData.type}) differs from registry type (${registryType})`);
          buildingType = object.userData.type;
        }
        
        console.log(`Object ${index} of registry type ${registryType}: final buildingType = ${buildingType}`);
    ...
    if (this.buildingSystem.builtWalls && this.buildingSystem.builtWalls.length > 0) {
      ...
      this.buildingSystem.builtWalls.forEach(wall => {
        // Check if this wall is already in our save list (avoid duplicates)
        const alreadyExists = allBuiltObjects.some(obj => 
  ...
  createBuildingMesh(buildingType, position, rotation) {
    ...
    const buildObject = this.buildingSystem.buildableObjects[actualBuildingType];
  ...
  restoreBuilding(buildingData) {
    ...
    if (!this.buildingSystem.buildableObjects || Object.keys(this.buildingSystem.buildableObjects).length === 0) {
    ...
    const actualType = typeof type === 'object' && type.type ? type.type : type;
